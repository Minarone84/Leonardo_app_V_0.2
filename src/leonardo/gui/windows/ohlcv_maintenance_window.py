"""GUI shell for canonical OHLCV Maintenance discovery and validation."""
...
class OhlcvMaintenanceWindow(QWidget):
    ...
    The window displays presenter-supplied rows and reports. It does not walk
    storage, parse CSV data, validate datasets, or write sidecars.
    """
    ...
    def _build_layout(self) -> None:
        ...
        apply_identity(
            datasets_panel,
            "ohlcv_maintenance.panel.datasets",
            object_type="panel",
        )
        ...
        configure_table(
            datasets,
            _DATASET_COLUMNS,
            object_id="ohlcv_maintenance.table.datasets",
        )
        ...
        apply_identity(
            notes_label,
            "ohlcv_maintenance.label.discovery_notes",
            object_type="label",
        )
    ...
    def _build_evidence_panel(self) -> QWidget:
        ...
        configure_table(
            table,
            _DETAIL_COLUMNS,
            object_id="ohlcv_maintenance.table.evidence",
        )
    ...
    def _build_issues_panel(self) -> QWidget:
        ...
        configure_table(
            table,
            _ISSUE_COLUMNS,
            object_id="ohlcv_maintenance.table.issues",
        )
    ...
    def _build_controls(self) -> QWidget:
        ...
        repair = self._add_button("repair", "Repair Selected", enabled=False)
        repair.setToolTip("Repair is intentionally unavailable until Task 1005.")
        refresh.clicked.connect(self.refresh_requested.emit)
        validate.clicked.connect(self.validate_requested.emit)
        cancel.clicked.connect(self.cancel_requested.emit)
        apply_identity(
            self._progress,
            "ohlcv_maintenance.progress.validation",
