...
def test_dropdowns_cascade_left_to_right_and_filter_the_table(qapp) -> None:
    ...
    try:
        ...
        assert table.item(0, 0).text() == "bybit"
        assert table.item(0, 1).text() == "linear"
        assert table.item(0, 2).text() == "LINKUSDT"
        assert table.item(0, 3).text() == "1h"
    finally:
        dialog.close()
...
def test_two_text_searches_combine_with_dropdowns_using_and(qapp) -> None:
    ...
    try:
        ...
        QCoreApplication.processEvents()
        assert table.rowCount() == 1
        assert table.item(0, 2).text() == "XRPUSD"
    finally:
        dialog.close()
...
def test_all_button_resets_every_filter_and_restores_full_catalog(qapp) -> None:
    ...
    try:
        ...
        assert len(rows) == 1
        assert table.item(rows[0].row(), 2).text() == "LINKUSDT"
    finally:
        dialog.close()
...
def test_active_dataset_uses_one_row_matching_table_and_hides_when_absent(qapp) -> None:
    ...
    try:
        ...
        assert _header_texts(active) == _header_texts(catalog)
        rows = catalog.selectionModel().selectedRows()
        assert len(rows) == 1
        assert catalog.item(rows[0].row(), 2).text() == "BTCUSDT"
...
def test_human_utc_rejected_gating_and_exact_selection_emission(qapp) -> None:
    dialog = DataManagerDatasetSelectorDialog()
    ...
    try:
        ...
        QCoreApplication.processEvents()
        assert "missing close column" in table.item(rejected_row, 10).text()
