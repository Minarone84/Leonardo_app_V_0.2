...
def test_main_window_metadata_documents_download_launch_surfaces() -> None:
    ...
    assert download_data["target_module_id"] == "connection.gui_home"
    assert download_data["target_window_id"] == "connection_suite.home.window"
    assert download_data["target_workflow_id"] == "connection_suite_shell"
    assert download_data["boundary_id"] == "download_data_boundary"
    assert download_data["permission_ref"] == "download:view"
    assert download_data["status"] == "shell_only"
    assert download_data["implementation_status"] == "opens_connection_suite_shell"
    ...
    assert ohlcv_maintenance["status"] == "shell_only"
